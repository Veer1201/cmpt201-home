...
Ans: The client sends data using the TCP protocol (SOCK_STREAM).

2. What data is the client sending to the server?
Ans: The client sends text strings packed into 1024-byte buffers.

Understanding the Server:
1. Explain the argument that the `run_acceptor` thread is passed as an argument.
Ans: It is passed a pointer to a struct acceptor_args. This structure contains
the run flag (to control the loop), and pointers to the shared resources (the
linked list and the mutex) so the acceptor can pass them down to the client
threads.

2. How are received messages stored?
Ans: Messages are stored in a singly linked list where each node contains the
message data.

3. What does `main()` do with the received messages?
Ans: It iterates through the linked list to print every message to the screen,
verifies that the total count matches the expected number, and frees the memory
for every node.

4. How are threads used in this sample code?
Ans: The server uses a "Thread-per-Client" model. There is one main thread, one
dedicated acceptor thread to listen for connections, and a separate thread is
created for every single client that connects to handle its data.
